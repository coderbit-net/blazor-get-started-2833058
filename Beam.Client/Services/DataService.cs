...
namespace Beam.Client.Services
{
    public class DataService
    {
        ...
        private BeamApiServices _apiService;
        ...
        public DataService(BeamApiServices apiService)
        {
            _apiService = apiService;
            if (CurrentUser == null)
                CurrentUser = new User() { Name = "Anon" + new Random().Next(0, 10) };
        }
        ...
        public async Task PrismRay(int RayId)
        {
            if (CurrentUser.Id == 0)
                await GetOrCreateUser();
            Rays = await _apiService.PrismRay(
                new Prism() { RayId = RayId, UserId = CurrentUser.Id }
            );
            UpdatedRays?.Invoke();
        }
